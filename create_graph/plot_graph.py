...
def configure_graph(ax, resampled_data, graph_title, valid_start_time, valid_end_time):
# def configure_graph(ax, resampled_data, graph_title, valid_start_time, end_time):
    ...
    ax.text(0.01, 0.01, f"開始日時: {valid_start_time.strftime('%Y-%m-%d %H:%M:%S')}\n終了日時: {valid_end_time.strftime('%Y-%m-%d %H:%M:%S')}\n合計時間: {round((valid_end_time - valid_start_time).total_seconds() / 60)}分",
            transform=ax.transAxes, fontsize=9, verticalalignment='bottom')
   
    ax.grid(True)

def plot_graph(resampled_data, valid_start_time, end_time, graph_title, png_file_name):
    # グラフのフレームと軸を作成する。ここでのグラフの大きさは幅15インチ、高さ8インチと定義する。
    fig, ax = plt.subplots(figsize=(15, 8))
    # 平均脳波のデータラインを軸にプロットするための関数を呼び出す。
    plot_average_lines(resampled_data, ax)
    # 各脳波の移動平均ラインを軸にプロットするための関数を呼び出す。
    plot_rolling_avg_lines(resampled_data, ax)
    # グラフのタイトル、軸のラベル、テキストボックスの配置など、グラフの視覚的な要素を設定する。
    configure_graph(ax, resampled_data, graph_title, valid_start_time, end_time)
    ...
    print("Create PNG file: ", png_file_name)
